...
class ListViewTest(TestCase):
    ...
    def post_invalid_input(self):
        ...
        return self.client.post('/lists/%d/' % (list_.id,), data={
            'text': ''
        })
    ...
    def test_for_invalid_input_shown_error_on_page(self):
        response = self.post_invalid_input()
        self.assertIn(escape(EMPTY_LIST_ERROR), response.content.decode())

    def test_duplicate_item_validation_errors_end_up_on_lists_page(self):
        list1 = List.objects.create()
        item1 = Item.objects.create(list=list1, text='blabla')
        response = self.client.post('/lists/%d/' % (list1.id,), data={'text': 'blabla'})
        self.assertIn(escape("You've already got this in your list"), response.content.decode())
        self.assertTemplateUsed(response, 'list.html')
...
class NewListTest(TestCase):
    ...
    def test_for_invalid_inut_passes_form_to_tamplate(self):
        response = self.client.post('/lists/new', data={'text': ''})
        self.assertIsInstance(response.context['form'], ItemForm)
